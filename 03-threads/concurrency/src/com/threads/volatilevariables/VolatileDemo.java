package com.threads.volatilevariables;


/*Notes on volatile:

    Whenever you declare any variable with volatile, it will always store it in main memory, not in cache or other palces
    While using local variables, they might be stored in cache or local storage, so when a thread reads the value
    of it, thread might not read upto date value because cache might not be upto date.
